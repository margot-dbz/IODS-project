...
library(lubridate)
typesdata_faulty$date <- ymd_hm(typesdata_faulty$date) # we need to correct the format in the dataset. 
```
...
near(measurement_mean, 3.6, 1)
near(measurement_mean, 3.6, 0.1)
```

### 2.2.2 Character variables

```{r}
library(tidyverse) # You can load the libraries at the beginning of your document, and they will be available for all subsequent chunks in the document. 
typesdata %>%
  count(group)

typesdata %>%
  count(group, sort = TRUE) # Sort the group based on the count from big to small
...
```{r}
my_datetime <- "2020-12-01 12:00"
my_datetime_converted <- ymd_hm(my_datetime) # to make it work I need to add dates to the column date that is empty in the dataset
...
#replace the NA in the date column from typesdata_faulty
typesdata_faulty$date <- as.POSIXct(typesdata_faulty$date, format = "%d.%m.%Y", tz = "UTC")
typesdata_faulty$date[is.na(typesdata_faulty$date)] <- as.POSIXct("30.12.2022", format = "%d.%m.%Y", tz = "UTC")
view(typesdata_faulty)
typesdata_faulty %>% 
...
```{r}
parsed_date <- parse_date_time("12:34 07/Jan'20", "%H:%M %d/%b'%y")

Sys.time() #Today time and time when code is run
Sys.time() %>% format("%H:%M on %B-%d (%Y)")

Sys.time() %>% format("Happy days, the current time is %H:%M %B-%d (%Y)!")

Sys.time() - parsed_date
```
...
seq(15, 30)
seq(15, a+1) # it print 15 to 104 included

example_sequence <- seq(15, 30)

example_sequence <- example_sequence/2

example_sequence # It will print (15/2) 7.5 to (30/2) 15

sequence <- seq(15, 30, by = 2) # It will print 15, 17, 19 ect
print(sequence)
```
...
print(mean_result)
mean_result <- mean(mydata$var2, na.rm <- TRUE) # does not work because of <- instead of =
...
```{r}
library(tidyverse)
mydata$var1 %>% mean() # print the value. Does not store the object.

mean_result <- mydata$var1 %>% mean() # Does not print but keep the output in an object we can reuse later
```
...
gbd_short %>% 
 filter(year = 1995) # whould be == and not =
...
gbd_short %>% 
  filter(year == max(year) | year == min(year), cause == "Injuries")
...
```{r}
mydata_year2000 <- gbd_short %>% # it does not print it but store it in an object
  filter(year == 2000)

mydata_year2000 # This prints the object that has been populated by the data filter by 2000
```
...
new_data_selection <- gbd_short %>% 
  filter((year == 1990 | year == 2011) & cause == "Communicable diseases")
...
mydata %>% 
  filter(var2 != 5 | is.na(var2)) # It will add NA rows + all rows different than 5 in var2
```
...
typesdata$"measurement/2" # It is impossible to print the data from this column as it doesnt have a name.
...
typesdata %>% 
  mutate(dates_difference = round(ymd_hm("2020-01-01 12:00") - date)) 
# This is not as easy to read the codes, it is preferable to break the codes in different lines.
```
...
typesdata %>% 
  ...
  select(plot_label) # here the pipe after the paste() is necessary as we want r to read all the line of codes at once.
...
pastedata %>% 
  mutate(date = paste(day, month, year, sep = "---")) %>% 
  mutate(date = dmy(date)) #It will take the data between the separators, first item is day, second item is month and third item is year.
```
...
patientdata
labsdata_updated
left_join(patientdata, labsdata_updated) #This one only adds data based on the IDs of the patientdata, so from ID 1 to 6. Because we add the second table, it will add one more row for 5 as there are 2 measurements. 
right_join(patientdata, labsdata_updated) #This one only adds data based on the IDs of the labsdata_updated so from ID 5 to 8 with 2 times the 5
```
...
gbd2017 %>% 
  ...
  ungroup() %>%  # Need to ungroup for the next calculations which is the total death all categories. 
  mutate(deaths_total = sum(deaths_per_group)) %>% 
  mutate(pourcentage= percent(deaths_per_group/deaths_total))
```
...
round(100*4.91/55.74, 1) %>% paste0("%") # paste0 do not add any space between the in t and string
...
```{r}
grouped_table <- full_join(gbd_summarised, gbd_summarised_sex)
grouped_table

grouped_table %>% 
  mutate(pourcentage_cause_sex = deaths_per_group/deaths_per_sex)

# or we can use mutate instead of combining the two tables.
gbd_summarised %>% 
  group_by(sex) %>% 
...
gbd2017 %>% 
  group_by(cause, sex) %>% 
  summarise(deaths_per_group = sum(deaths_millions)) %>% #keep the table we need with cause and sex and we add the total deaths for each subgroup
  group_by(sex) %>% # Then we group per sex
  mutate(deaths_per_sex  = sum(deaths_per_group), #add the total death per sex to compare with the total of death per subgroup (cause, sex)
         sex_cause_perc = percent(deaths_per_group/deaths_per_sex)) %>% #W calculate the ratio and use the percent to show the %. we cannot use this data to calulate anymore. It is a string
  arrange(sex, deaths_per_group) # we order per cause, then sex
```
...
```{r}
mynumbers <- c(1, 2, NA)
sum(mynumbers) #cannot calculate due to the NA value

sum(mynumbers, na.rm = TRUE) # we remove the NA and it calculates.
```
...
```{r}
gbd_wide %>% 
  pivot_longer(matches("Female|Male"), # here the | means OR
               names_to = "sex_year", 
...
gbd_factored$cause %>% levels() # This means that factors will be sorted based on the order in which their levels are encountered in the original data, not necessarily alphabetically.

gbd_factored <- gbd_factored %>% 
  mutate(cause = cause %>% 
           fct_relevel("Injuries")) #This will order the injuries first.
...
dim(gbd_full) # 168 entries

glimpse(gbd_full) # to inspect the structure of a dataset
...
year_count # 24 entries per year and there are 7 years = 168 as the number of entries in the dataset.
...
library(scales)
summary_data1_data2 <- full_join(summary_data1, summary_data2) %>% 
  mutate(distribution_cause_deaths_per_year = total_per_cause / total_per_year) %>% 
  mutate(percent(distribution_cause_deaths_per_year))
...
label_data2 <- gapdata2007 %>% 
  group_by(continent) %>%
  summarize(lifeExp = mean(lifeExp)) %>%
  mutate(lifeExp = round(lifeExp, 2)) %>%
...
gapdata2007 %>% 
  ...
  geom_jitter(aes(colour = continent)) +
  # Third geom - label, with its own dataset (label_data) and aes(label = )
  geom_text(data = label_data2, aes(label = lifeExp))
