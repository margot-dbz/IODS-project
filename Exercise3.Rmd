...
url_por <- paste(url, "student-por.csv", sep = "/") # this will add the text of url + the "students-por.csv" and will separate text with sep /
...
# give the columns that vary in the two data sets-> use the c("","","") to create a vector of column names
...
join_cols
# the set_dif is a function that helps to compare 2 vectors of column titles from the dataset por and the ones we want to remove, selected via the free_cols, it will then exclude the columns from free_cols in por dataset

# join the two data sets by the selected identifiers
math_por <- inner_join(math, por, by = join_cols)
# Then we do the join tables via only the similar columns from por and math
...
# Inner join with suffix argument, suffix is adding the suffix to the data that belongs to each data set por and math (G1.math and G1.por) for instance
math_por__suffix <- inner_join(math, por, by = join_cols, suffix = c(".math", ".por"))

# Print new column names after suffix addition
print(colnames(joined_data_suffix))

# Use glimpse to look at the joined data types
glimpse(joined_data_suffix)
...
# create a new data frame with only the joined columns. All of select all columns from the vector join_cols which were the ones that are common for both datasets.
...
# It will check all columns name from the table math_por that are in the vector free_cols. So fo instance here it will start with the vector failure, and check if the vector failure is in the math_por as columns, if there are many, it will select them all and save them in the two_cols. Then, it will take the first vector among the listed ones and create a new column in the table alc. If the data stored in first_col is numerical, it will just populate the new added column with the average of the 2 columns selected in two_cols (the ones that starts with the string failure). If it is a word, it will just use the first data to populate the cell.  
for(col_name in free_cols) {
  # select two columns from 'math_por' with the same original name
  two_cols <- select(math_por, starts_with(col_name))
  # select the first column vector of those two columns
  first_col <- select(two_cols, 1)[[1]] # the,1 = first column of vector that starts with the free_col column names  and then, [[1]], extracts this selected column as a vector and assigns it to the variable first_col.
...
# produce summary statistics by group. Here we want to calculte the statistic per sex, so we group the data per sex, then we do the total of the count per sex using the n()
...
# define the plot as a boxplot and draw it. rename the y with ylab("title")
...
# indicates that you're modeling a binary outcome, where the response variable takes only two possible outcomes (e.g., success/failure, yes/no, 1/0).

# compute odds ratios (OR). The odds ratio helps in understanding the association or relationship between the exposure (or treatment) and the outcome. If the odds ratio is 1, it suggests that there's no association between the exposure and outcome. If it's greater than 1, it indicates a higher odds of the event in Group 1 compared to Group 2. If it's less than 1, it indicates a lower odds of the event in Group 1 compared to Group 2. 
# So here we analyze the impact of different variables on the high_use of alcohol. high_use is the outcome variable or the dependent variable. The model estimates the effect of changes in the predictor variables (failures, absences, sex) on the odds of the outcome (high_use). It provides coefficients (log-odds) associated with each predictor to understand how each predictor influences the log-odds of the outcome occurring.
OR <- coef(m) %>% exp
OR
# compute confidence intervals (CI). When you calculate confidence intervals for the mean, you're estimating a range around the sample mean within which the true population mean is likely to lie. It's often expressed as "the sample mean plus or minus a margin of error" at a certain confidence level. from 2.5% to 97.5%
CI <- confint(m)
CI 
# print out the odds ratios with their confidence intervals. combine the 2 tables
cbind(OR, CI)
...
# predict() the probability of high_use. In the context of logistic regression, the "response" refers to the predicted probabilities of the binary outcome variable based on the model fitted using glm(). For instance, if you have a logistic regression model m predicting the probability of high_use based on predictors such as failures, absences, and sex, using predict() with type = "response" would give you the predicted probabilities of high_use being 1 (or the positive outcome) for each observation in your dataset alc.
probabilities <- predict(m, type = "response")
probabilities
library(dplyr)

# add the predicted probabilities to 'alc'. it gives the probability to be high_use of alcohol considering the criteria given in the regression mode.
alc <- mutate(alc, probability = probabilities)
alc

# use the probabilities to make a prediction of high_use. if >0.5, probability is TRUE
alc <- mutate(alc, prediction = probability > 0.5)

# see the last ten original classes, predicted probabilities, and class predictions
select(alc, failures, absences, sex, high_use, probability, prediction) %>% tail(10) # it retrieves the last 10 rows of the dataset or object.

# tabulate the target variable versus the predictions
table(high_use = alc$high_use, prediction = alc$prediction)
#This gives the number of False and True for the high_use real and then the prediction of high use 
...
# define the geom as points and draw the plot. To visualize false positive , false negative and true positive and true negative. the red color is for the wrong prediction so false positive and True negative. 
g + geom_point()
g + geom_point(aes(col = prediction))


table(high_use = alc$high_use, prediction = alc$prediction) 
# so the row1col1 is 252/ (252+78+ 7+33) = 0.68%

# tabulate the target variable versus the predictions
table(high_use = alc$high_use, prediction = alc$prediction) %>% 
  prop.table() %>%  #This function converts the frequencies in the contingency table into proportions. By default, it calculates proportions over the entire table (i.e., the grand total).
  addmargins() # add sum to the columns and rows
