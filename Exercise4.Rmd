...
class(boston_scaled) #QUESTION, WHAT DOES IT MEAN
...
boston_scaled$crim <- as.numeric(boston_scaled$crim) # Take only numeric values
```
...
correct_classes <- test$crime # to get classes and not a table
# question: what is the difference between the 2?

# remove the crime variable from test data
test_no_crime <- dplyr::select(test, -crime)
...
Question: what is the difference between a vector and other data in r?
...
# target classes as numeric - this gives the number of classes for the continuous variable so for instance: low = 1, med low = 2, med high = 3, high = 4)
...
# k-means clustering - centers is the number of clusters that we want to get
...
# question: why using the $cluster?
