...
# select the 'keep' columns. One_of is used to select all the columns in keep
...
# filter out all rows with NA values, all the true in complete.cases(human)
human_ <- filter(human, complete.cases(human)) # modify the "TRUE", see instructions above!
#The presence of missing values can bias the result of PCA. Therefore, it is highly recommended to perform the appropriate approach to tackle those values. 
```
...
# move the country names to rownames, this means that the column country will be the first col as "title rows".
library(tibble)
...
# standardize the variables - mean is equal 0
...
pca_human_test <- prcomp(human_std)
?prcomp
# draw a biplot of the principal component representation and the original variables
biplot(pca_human_test, choices = 1:2)
?biplot
...
# visualize the dataset - How to create bars for each categorical variables
library(ggplot2)
